...
function mapProperty(row: RawPropertyRow): HostProperty {
  const sorted = [...(row.property_images ?? [])].sort(
  ...
  return {
    ...
    status: row.status as HostProperty["status"],
    instant_book: row.instant_book,
    rating_avg: row.rating_avg,
    rating_count: row.rating_count,
    min_price_fcfa: row.min_price_fcfa,
...
export function useHostProperties(): UseHostPropertiesReturn {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setProperties(((data ?? []) as RawPropertyRow[]).map(mapProperty));
      setLoading(false);
    }
