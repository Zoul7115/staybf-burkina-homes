...
type RawRow = {
  id: string; amount_fcfa: number; currency: string; status: string; method: string | null;
...
async function fetchAdminPayments(): Promise<AdminPaymentRow[]> {
  ...
  const { data, error: dbErr } = await (supabase as any)
    .from("payments")
    .select(`id,amount_fcfa,currency,status,method,created_at,captured_at,bookings!booking_id(reference,profiles!traveler_id(full_name))`)
  ...
  return ((data ?? []) as RawRow[]).map((p) => {
    ...
    return {
      ...
      method: p.method, amountFcfa: p.amount_fcfa, currency: p.currency ?? "XOF",
