...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: property, error: fetchErr } = await db.from("properties").select("id, host_id").eq("id", property_id).single();
    if (fetchErr || !property) return err("Property not found", 404);

    const { error: updateErr } = await db.from("properties").update({ status: "rejected" }).eq("id", property_id);
    ...
    await db.from("notifications").insert({
      ...
      body: reason ?? "Votre propriété n'a pas été approuvée.",
      data: { property_id },
    });
